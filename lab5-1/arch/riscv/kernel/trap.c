    ...
    if (cause == 0xc || cause == 0xd || cause == 0xf) {
      ...
      list_for_each_entry(vma, &current->mm.vm->vm_list, vm_list) {
        ...
        if(stval>=vma->vm_start && stval<vma->vm_end){
          uint64_t need_perm = PTE_V | PTE_U;
          if(cause == 0xc){
            need_perm |= PTE_X;
          }else if(cause == 0xd){
            need_perm |= PTE_R;
          }else if(cause == 0xf){
            need_perm |= PTE_R | PTE_W;
          }
          if( (vma->vm_flags & need_perm) == need_perm ){
            ...
            create_mapping((current->satp)<<20>>8,vma->vm_start,pa,(vma->vm_end-vma->vm_start),vma->vm_flags);
            vma->mapped = 1;
            return;
          }else{
            printf("need %llx permission, but have %llx permission\n",need_perm, vma->vm_flags);
            sp_ptr[16] += 4;
            return;
          }
        }
