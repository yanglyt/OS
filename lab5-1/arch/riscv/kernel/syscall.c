...
struct ret_info syscall(uint64_t syscall_num, uint64_t arg0, uint64_t arg1, uint64_t arg2, uint64_t arg3, uint64_t arg4, uint64_t arg5) {
    ...
    case SYS_MMAP: {
        ...
        new_vma->vm_start = arg0;
        new_vma->vm_end = arg0 + arg1;
        new_vma->vm_flags = arg2;
        new_vma->mapped = 0;
        list_add(&(new_vma->vm_list), &(current->mm.vm->vm_list));
        ret.a0 = new_vma->vm_start;
        break;
    ...
    case SYS_MUNMAP: {
        ...
        list_for_each_entry(vma, &current->mm.vm->vm_list, vm_list) {
            if(vma->vm_start == addr && vma->vm_end == addr + length){
                if(vma->mapped){
                    for(uint64_t va = vma->vm_start; va < vma->vm_end; va += PAGE_SIZE){
                        uint64_t pte = get_pte(((current->satp)<<20)>>8, va);
                        if(pte & PTE_V){
                            uint64_t pa = (pte >> 10) << 12;
                            free_pages(pa);
                        }
                        create_mapping(((current->satp)<<20)>>8, va, 0, PAGE_SIZE, 0);
                    }
                }
